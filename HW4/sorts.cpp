...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    {
        vector<Student>::iterator nextItem = s.begin() + i;
        int loc = i;
        while ( loc > 0 && comp(*nextItem, *(nextItem - 1)) )
        {
            swap(*nextItem, *(nextItem - 1));
            nextItem--;
            loc--;
        }
    }
...
int main()
{
    ...
    // destruction time.
    {
        ...
        vector<Student>::iterator iter = auxStudents.begin();
        while (iter != auxStudents.end())
        {
            studentPtrs.push_back(&(*iter));
            iter++;
        }
        ...
        vector<Student*>::iterator sorted = studentPtrs.begin();
        for (int i = 0; i < students.size(); i++)
        {
            swap(students[i], **sorted);
            sorted++;
        }
